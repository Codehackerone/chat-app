...
function toggle_status(id_element, change_to) {
  if (change_to == 0) {
    ...
    document.querySelector("#" + id_element).className =
      "border border-green-300 shadow p-3 w-full rounded";
  } else if (change_to == 1) {
    //wrong
    document.querySelector("#" + id_element + "-correct").hidden = true;
    document.querySelector("#" + id_element + "-wrong").hidden = false;
    document.querySelector("#" + id_element).className =
      "border border-red-300 shadow p-3 w-full rounded";
  } else {
    //neutral
    document.querySelector("#" + id_element + "-correct").hidden = true;
    document.querySelector("#" + id_element + "-wrong").hidden = true;
    document.querySelector("#" + id_element).className =
      "border border-gray-300 shadow p-3 w-full rounded";
  }
}
...
function checkdob() {
  ...
  toggle_status(
    "dob",
    new Date().getFullYear() - new Date(dob).getFullYear() < 15 ? 1 : 0
  );
  arr_err[1] =
    new Date().getFullYear() - new Date(dob).getFullYear() < 15 ? true : false;
  checkerrors();
}
...
function checkerrors() {
  document.getElementById("submit").disabled =
    allEqual(arr_err) && !arr_err[0] ? false : true;
}
